...
Plusieurs sous-fonctions nous ont été utiles pour la génération de graphes, notamment pour garantir la connexité dans tous les algorithmes :
\BlankLine
\begin{itemize}
...
\Entree{Un nombre $n$ de sommets à générer et une probabilité $\proba$ de connexion entre chaque couple de sommet}
...
Comme dit précédemment, l'algorithme travaille sur un ordre aléatoire des sommets (et non pas sur les vrais indices). Pour des raisons de simplicité, nous considérerons dorénavant dans nos explications, que les nombres manipulés représentent directement les indices des sommets.

La variable \texttt{estConnecté} sera mise à jour pour qu'à la fin de la deuxième boucle \texttt{for} elle indique si le sommet courant (d'indice $i$) est connecté à un des sommets précédents (d'indice dans $j\in[0, i[$).
...
\Entree{Un nombre $n$ de sommets à générer}
...
Nous connectons chaque sommet d'indice $i$ à un unique sommet d'indice $j\in[0, i[$ (ce sommet peut être vu comme le "père" de ce sommet et $j$ n'est pas une variable explicitement définie dans l'algorithme).
...
\Entree{Un nombre $n$ de sommets à générer et une probabilité $\proba$ de connexion entre tout couple de sommets}
...
création d'un tableau extensible \texttt{part1} de taille $\size 1=0$

création d'un tableau extensible \texttt{part2} de taille $\size 2=0$

\bigskip
ajouter(part1, ordreSommets[$0$])

ajouter(part2, ordreSommets[$1$])

ajouterArête(G, ordreSommets[$0$], ordreSommets[$1$])
...
Les tableaux extensibles représentent les deux partitions de $G$. On les initialise arbitrairement avec le sommet d'indice $0$ dans la première partition et le sommet d'indice $1$ dans la seconde, puis on les relie (on part ainsi avec une composante connexe et deux partitions non vides).

Ensuite, pour chaque nouveau sommet, on l'ajoutera aléatoirement dans la première ou dans la seconde partition (grâce au tirage \texttt{random} $<$ 0.5). On utilisera la même méthode que précédemment pour s'assurer d'avoir des sommets connectés à la composante connexe déjà présente (\texttt{estConnecté} et \texttt{makeConnected}), tout en veillant à ne les connecter qu'à des sommets de la partition opposée.
...
\Entree{Un nombre $n$ de sommets à générer, une probabilité $\proba$ de connexion entre tout couple de sommets et un entier $k$ représentant la taille de couverture souhaitée}
...
Durant la première boucle, nous parcourons tous les sommets de la couverture (fixés arbitrairement comme les $k$ premiers) et nous effectuons deux opérations :
\begin{itemize}
...
création d'un tableau de booléens \texttt{estVisité} de taille $n$ avec toutes ses valeurs à false
...
Cet algorithme calcule la bi-partition d'un graphe biparti connexe. Pour ce faire, il effectue tout simplement un parcours en largeur en se rappelant quels sommets il n'a pas encore \og rangé\fg et quels sommets il a déjà \og visité\fg.

Le premier sommet ($0$) est fixé arbitrairement dans la partition 1, puis chaque voisin d'un sommet à tester sera dans sa partition opposée (1-partitionCourante).


\vspace{0.35cm}
\begin{algorithm}[H]
\caption{Algorithme de construction du graphe orienté $H$}
\Entree{Un graphe $G$ biparti connexe et ses deux partitions part1 et part2 de tailles respectives size1 et size2 (tableaux d'entiers représentant les indices)}
...
Si on appelle $X$ et $Y$ les deux partitions de $G$, alors l'algorithme effectue le travail suivant :
\begin{itemize}
\itemb il crée un graphe orienté $H$ de taille $n+2$ (deux nouveaux sommets : $s$ et $t$)
\itemb il relie $s$ vers tous les sommets de $X$
\itemb il ajoute toutes les arêtes de $G$ orientées de $X$ vers $Y$
\itemb il relie tous les sommets de $Y$ vers $t$
\end{itemize}

Ce graphe est nécessaire pour l'algorithme trouvant une couverture optimale pour les graphes bipartis. La prochaine étape est d'y appliquer l'algorithme de Ford-Fulkerson pour trouver un flux optimal allant de $s$ vers $t$ (toute les arêtes ayant une capacité de 1).
\\
\begin{algorithm}[H]
\caption{Algorithme de Ford-Fulkerson}
\Entree{Le graphe orienté $H$ et les deux partitions part1 et part2 de G de tailles respectives size1 et size2 (tableaux d'entiers représentant les indices)}
...
création d'une matrice d'entiers \texttt{flux} de taille $n\times n$ avec toute les valeurs initialisées à 0
...
Le tableau de couleurs utilisé représente les couleurs de l'algorithme (0 pour blanc, 1 pour gris et 2 pour noir).
...
On remarquera que l'on pourra passer par des flux déjà à 1 mais seulement en allant d'un fils vers son père (cela correspond à trouver un chemin valide dans le graphe résiduel) ce qui permettra d'optimiser le flux.

Finalement, on ajoute dans la couverture tous les sommets dans la première partition coloriés en blanc et tous les sommets de la deuxième partition coloriés en noir.
...
L'algorithme de Ford-Fulkerson a, quand à lui, une complexité de $O(|E(G)|)$ puisque toutes les capacité d'arêtes sont fixées à 1.

Nous pouvons donc en déduire que la complexité de l'algorithme de calcul de couverture minimale pour un graphe biparti, qui cumule ces trois algorithmes, est en $O(|E(G)|+ |V(G)|)$.
...
Nous avons comparé la taille de la couverture trouvée avec la taille de la couverture effective (sur des graphes générés avec \texttt{littleGen}) : l'algorithme \texttt{edgesDeletion} renvoie bien des couvertures de taille inférieure à deux fois celle de la couverture optimale.
...
\bigskip
Nous avons ensuite testé l'algorithme sur des séries de 100 arbres générés aléatoirement avec \texttt{tree} \texttt{Generation} ; ces graphes ayant moins d'arêtes, nous nous attendons à pouvoir traiter des graphes de taille plus grande.
...
Les deux versions (itérative et récursive) ont été exécutées sur un ensemble de 100 graphes de $n$ sommets (générés aléatoirement avec la fonction \texttt{generation} et une probabilité de 0.5), sur une même machine.
...
Les tests sont : 
\begin{itemize}
\itemb si $k$ est supérieur à la taille du graphe, alors tous les sommets sont dans la couverture.
\itemb si $k$ est négatif, alors le graphe n'a pas de couverture de taille $k$.
\itemb si le graphe comporte plus de $k\times(n-1)$ arêtes, alors il n'a pas de couverture de taille $k$.
\end{itemize}
...
Nous nous proposons maintenant de trouver une couverture d'une taille donnée si elle existe, à l'aide du sat-solver \texttt{minisat}, en passant donc par une réduction du problème Vertex Cover.
...
Ces contraintes se formulent en problème de satisfaisabilité de la façon suivante :
...
Le problème de satisfaisabilité auquel se réduit Vertex Cover est finalement $\varphi=\varphi_1\land \varphi_2\land \varphi_3$.
...
On réduit ainsi la recherche d'une $p$-couverture sur $n$ sommets à un problème de $2p$-satisfaisabilité à $\frac {np (n+p-2)}2 +|E|$ clauses. La complexité de la réduction en elle-même est proportionnelle au nombre de clauses générées, mais elle est négligeable par rapport à la résolution du problème de satisfaisabilité.
