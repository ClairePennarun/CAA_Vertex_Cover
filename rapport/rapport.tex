...
Plusieurs sous-fonctions nous ont été utiles pour la génération de graphe, notamment pour garantir la connexité dans tout les algorithmes :
\BlankLine
\begin{itemize}
...
\Entree{Un nombre $n$ de sommets à générer et une probabilité $\proba$ de connexion entre tout couple de sommet}
...
\Entree{Un nombre $n$ de sommets à générer, une probabilité $\proba$ de connexion entre tout couple de sommet et en entier $k$ représentant la taille de couverture souhaitée}
...
\begin{itemize}
\itemb il crée du graphe orienté $H$ de taille $n+2$ (deux nouveaux sommets : $s$ et $t$)
\itemb il relie $s$ vers tout les sommets de $X$
\itemb il ajoute toute les arêtes de $G$ orientée de $X$ vers $Y$
\itemb il relie tout les sommets de $Y$ vers $t$
\end{itemize}
...
Nous pouvons donc en déduire que la complexité de l'algorithme de calcul de couverture minimale pour un graphe biparti, qui cumule ces trois algorithmes, est %linéaire en la taille du graphe ($O(|E(G)|+ |V(G)|)$).
en $O(|E(G)|+ |V(G)|)$.
